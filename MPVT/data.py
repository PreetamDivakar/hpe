...
class dataflow(Logger):
    ...
    def get_total_offset(self):
        command = f"/home/preetam/kafka/kafka_2.13-3.9.0/bin/kafka-consumer-groups.sh --bootstrap-server localhost:9092 --describe --group logstash-group"

        try:
            result = subprocess.run(command, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, universal_newlines=True, check=True, env={'PATH': '/usr/local/bin:/usr/bin:/bin:/home/preetam/kafka/kafka_2.13-3.9.0/bin'})
    ...
    def check_events_in(self):
        ...
        if response and response.status_code == 200:
            ...
            try:
                self.log_error(data)
                current_events_in = data['pipelines']['main']['events']['in']
    ...
    def kafka_data(self):
        ...
        def on_message(client, userdata, msg):
            try:
                payload = json.loads(msg.payload.decode('utf-8'))
                kafka_producer.produce(json.dumps(payload).encode('utf-8'))
                print(f"[Bridge] Forwarded MQTT → Kafka: {payload}")
            except Exception as e:
                print(f"Error producing to Kafka: {str(e)}")
